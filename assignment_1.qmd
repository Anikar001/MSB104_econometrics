...
What does the GINI really tell us? (Look at paper from page 116)
...
```{r}
#using R and the dplyr package to manipulate a dataset and calculate GDP per capita at the NUTS2 region level for the year 2010
...
#using R and the dplyr package to load and manipulate three different datasets related to demographics, transport infrastructure, and education at the NUTS2 regional level. Below is a breakdown of the steps and transformations you are performing on each dataset:
...
```{r}
#Integrating different datasets into a comprehensive dataframe named Inequity_Data, and subsequently creating separate dataframes for specific countries based on their NUTS (Nomenclature of Units for Territorial Statistics) codes and the year 2010. Here's a breakdown of the code:
...
```{r}
#creating linear regression models for seven different datasets, each corresponding to a different country or the entire dataset, to analyze the relationship between regional inequality (measured by the GINI_NUTS2 variable) and per capita GDP (measured by the GDP_capita_NUTS2 variable) for the year 2010.

#lm1 = lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = PT2010)
#lm2 = lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = FR2010)
#lm3 = lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = HU2010)
#lm4 = lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = SK2010)
#lm5 = lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = DK2010)
#lm6 = lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = Inequity_Data)
```

Table XXX

```{r}
#using the huxreg() function to create a table summarizing the results of several linear regression models. The huxreg() function is typically used with the huxtable package in R, which is a tool for creating LaTeX and HTML tables from R objects.

#huxreg(list("PT" = lm1, "FR" = lm2,  "HU" = lm3, "SK" = lm4, "DK" = lm5, "CC" = lm6), statistics = c(N = "nobs", R2 = "r.squared"), coefs = c(GDP_capita_NUTS2 = "GDP_capita_NUTS2", const. = "(Intercept)"), note = "Note: {stars} T statistics in brackets.")
```

PLOTTING

```{r}
#creating a scatter plot of the GINI_NUTS2 against GDP_capita_NUTS2 using data from Inequity_Data. In addition to the scatter plot, a regression line (calculated from a previous simple linear regression model, presumably) is added to the plot.

#| label: fig-17
#| fig-cap: "Gini & GDP per capita CC"
#Inequity_Data %>%
  #ggplot(aes(x = GDP_capita_NUTS2, y = GINI_NUTS2)) +
  #geom_point(color = "maroon", alpha = 0.7) +
  #geom_abline(intercept = b0_GDP_on_GINI_IT, slope = b1_GDP_on_GINI_IT, color = #"blue", alpha = 0.5) +
  #labs(x = "GDP per Capita", y = "GINI") +
  #theme_bw()

```

## **III. Residuales / model interpetation?**

```{r}
#predicting the GINI coefficient based on GDP per capita for all the regions in Inequity_Data using the lm_GDP_on_GINI model, and then adding these predicted values and residuals to the Inequity_Data dataframe.

# Fit a simple linear regression model
#lm_GDP_on_GINI <- lm(GINI_NUTS2 ~ GDP_capita_NUTS2, data = Inequity_Data)

# Predict the GINI coefficient and add predictions and residuals to the dataframe
...
```{r}
#| label: fig-18
#| fig-cap: "Gini & GDP per capita CC with residuals"
...
#defines a function named MRM_NUTS2 that performs multiple linear regression analyses on a given dataset (mlrdata). This function creates five linear models, each with different sets of predictors, and then displays the summary of these models in a nicely formatted table using the huxreg function from the huxtable package.

#MRM_NUTS2 <- function(mlrdata){
  #lm8 = lm(GINI_NUTS2 ~ Tertiary_educ, mlrdata)
  #lm9 = lm(GINI_NUTS2 ~ Number_of_MWAY_and_RL, mlrdata)
  #lm10 = lm(GINI_NUTS2 ~ GDP_capita_NUTS2 + Tertiary_educ + Number_of_MWAY_and_RL, #mlrdata)
  #lm11 = lm(GINI_NUTS2 ~ Life_expectency, mlrdata)
  #lm12 = lm(GINI_NUTS2 ~ GDP_capita_NUTS2 + Tertiary_educ + Life_expectency, #mlrdata)
