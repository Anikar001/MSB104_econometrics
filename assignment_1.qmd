...
$\text{GDP per capita = } \beta_{0} + \beta_{1} \cdot GINI + \in$

```{r}
#| echo: false
#| output: false

# Making a dataframe that only contains observations from 2010, also filter the data so that gini is bigger than 0. 

data2010 <- gini_nuts_2 %>% filter(year == 2010, gini_n2 > 0)
```

```{r}
#| echo: false
#| fig-cap: Descriptive statistics all countries for year 2010
...
# lm(y ~ x) y = GDP per capita, x = Gini, estimation
lm_centered <- lm(gini_n2 ~ log_GDP_per_capita, data = data2010)

#Save coefficients
b0_earn <- lm_centered$coefficients[1]
b1_earn <- lm_centered$coefficients[2]

# Descriptive statistics
huxreg(All = lm_centered, statistics = c("r.squared","adj.r.squared", "statistic", "p.value"))
```
...
#| output: false
# må forklare denne!!
plot(lm_centered, which = 1:4)
```

β0​ = 0.0551 (intercept)

β1 = 0.0193 (slope coefficient)

```{r}
#| echo: false
#| fig-cap: Descriptive statistics for all countries seperately for the year 2010

# Finding estimates for each country seperately 
...
# Descriptive statistics
huxreg(SK = lmSK, DK = lmDK, HU = lmHU, PT = lmPT, FR = lmFR, statistics = c("r.squared","adj.r.squared", "statistic", "p.value"))
```

### 2. Model Diagnostics

Forklaring på statistikk for alle land samlet:

Forklaring på statistikk for land hver for seg:
...
ggplot(data2010, aes(x = gini_n2, y = log_GDP_per_capita)) +
  geom_point(aes(color = gini_n2), size = 3) +
...
Forklar hvorfor noen observasjoner er langt unna de andre observasjonene.
...
demo_dens_data <- read_csv("Raw_data/demo_dens.gz")
...
demo_dens_data <- demo_dens_data |> 
  select(nuts_2 = geo, pop_density = OBS_VALUE)

educ_data <- educ_data |> 
  select(nuts_2 = geo, students_percentage = OBS_VALUE)

rail_data <- rail_data |> 
  select(nuts_2 = geo, rail_km = OBS_VALUE)
```

```{r}
merged_data_1 <- demo_dens_data %>%
  inner_join(educ_data, by = 'nuts_2') %>%
  inner_join(rail_data, by = 'nuts_2')
```
